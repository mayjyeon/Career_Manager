...
**세특 내용** 칸과, 학생을 가리키는 칸만 있으면 읽습니다.
학생을 가리키는 방법은 학교마다 달라 두 가지를 모두 받습니다.

```
 ① 학번이 한 칸                      ② 학년마다 파일이 따로 (반·번호가 나뉨)
 ┌───────┬──────┬───────────┐       ┌──────┬──────┬──────┬───────────┐
 │ 학번  │ 이름 │  교과세특  │       │ 반   │ 번호 │ 이름 │  교과세특  │
 ├───────┼──────┼───────────┤       ├──────┼──────┼──────┼───────────┤
 │ 10203 │박바다│ 동아리에서…│       │  2   │  3   │박바다│ 동아리에서…│
```

- 열 순서는 상관없습니다. 머리글 **이름**으로 찾습니다.
- **세특 내용** 칸은 `교과세특`·`세특`·`세부능력 및 특기사항`·`활동내용` 등으로 적어도 읽고,
  `과목세특` 처럼 앞뒤에 말이 붙어도 알아봅니다.
- **자리**는 `학번`(5자리 = 학년 1 + 반 2 + 번호 2) 한 칸이거나, `학년`·`반`·`번호` 를 나눠
  적어도 됩니다. `성명`·`출석번호` 처럼 달리 적어도 읽습니다.
- **학년·반이 표에 없으면** 제목 줄(`2026학년도 1학년 교과세특`)이나 시트 이름(`1학년`, `1-2`)
  에서 찾아봅니다. 그래도 없으면 미리보기에서 직접 채웁니다.
- **바이트 수 칸은 읽지 않고 버립니다**(아래 참고).
- **모르는 칸(과목·학기·비고 등)은 이름 그대로 보관**했다가 내보낼 때 함께 내보냅니다.
  몇 개를 찾았는지 미리보기에서 알려 줍니다.
- 파일 안에서 학번이 뒤죽박죽이어도 학년·반·번호 순으로 정렬해 보여 줍니다.
- 시트가 여러 개면 모두 읽습니다. 시트 이름을 학년·반으로 쓰지 않은 경우에만
  어디서 왔는지 잃지 않도록 `시트` 칸을 덧붙입니다.

칸을 못 찾으면 **무엇이 없었는지 짚어서** 알려 줍니다
(예: `세특 내용(교과세특) 칸을 찾지 못했습니다`).
...
| 세특 엑셀에서 칸을 못 찾음 | 안내에 적힌 칸을 확인. 자리는 `학번` 한 칸이거나 `반`+`번호` 두 칸이면 됩니다 |
| 세특이 모두 ‘학년 값을 알 수 없습니다’ | 표에 학년 칸이 없는 파일입니다. 미리보기의 `학년` 을 채워주세요 |
