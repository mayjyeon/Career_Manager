...
학생 관리·상담일지·포트폴리오·휴지통은 **같은 모양의 검색 막대**를 씁니다
...
- **학생 삭제** — 학생 관리에서 여러 명을 골라 한 번에 지웁니다(자퇴·졸업 등).
  소속·상담 기록·선생님이 등록한 포트폴리오도 함께 들어가며, 되살리면 같이 돌아옵니다.
...
| `meta.owns` | `store.js` (선생님 계정 안) | `portfolios` |
...
views/*  ─ 화면. 데이터 계층을 직접 부르지 않고 아래를 거칩니다.
  │
  ├─ services.js ─ 선생님 자료(학생·상담·선생님이 등록한 포트폴리오)의 업무 규칙
  │      └─ store.js  users/{uid}/… (반 문서 · 상담 기록 · 포트폴리오)
